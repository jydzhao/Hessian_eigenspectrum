...
# Create model of linear NN with L hidden layers and varying number of hidden units
...
# Create model of ReLU NN with L hidden layers
# input dim = d, hidden dim = m, output dim = k
class ReLU_NN(nn.Module):
    ...
    def init_weights(self, init_type):
        if init_type == 'kaiming_normal':
            torch.nn.init.kaiming_normal_(self.lin_in.weight, nonlinearity='relu')
            torch.nn.init.kaiming_normal_(self.lin_out.weight, nonlinearity='relu')
            for i in range(self.L):
                torch.nn.init.kaiming_normal_(self.lin_hidden[i].weight, nonlinearity='relu')
        elif init_type == 'kaiming_uniform':
            torch.nn.init.kaiming_uniform_(self.lin_in.weight, nonlinearity='relu')
            torch.nn.init.kaiming_uniform_(self.lin_out.weight, nonlinearity='relu')
            for i in range(self.L):
                torch.nn.init.kaiming_uniform_(self.lin_hidden[i].weight, nonlinearity='relu')
        elif init_type == 'xavier_normal':
            torch.nn.init.xavier_normal_(self.lin_in.weight)
            torch.nn.init.xavier_normal_(self.lin_out.weight)
            for i in range(self.L):
            ...
            torch.nn.init.xavier_uniform_(self.lin_in.weight, nonlinearity='relu')
            torch.nn.init.xavier_uniform_(self.lin_out.weight, nonlinearity='relu')
            for i in range(self.L):
                torch.nn.init.xavier_uniform_(self.lin_hidden[i].weight, nonlinearity='relu')
        else:
            print('Unknown initialization. Using Kaiming normal initialization')
            torch.nn.init.kaiming_normal_(self.lin_in.weight, nonlinearity='relu')
            torch.nn.init.kaiming_normal_(self.lin_out.weight, nonlinearity='relu')
...
# Create model of ReLU NN with L hidden layers and varying number of hidden units
...
# Create model of linear NN with L hidden layers
# input dim = d, hidden dim = m, output dim = k
class Linear_fully_skip_NN(nn.Module):
    def __init__(self,d,m,k,L,beta=1):
        """
            d: input dimension
            m: hidden layer dimension 
...
# Create model of linear NN with L hidden layers
# input dim = d, hidden dim = m, output dim = k
class ReLU_fully_skip_NN(nn.Module):
    def __init__(self,d,m,k,L,beta=1):
        """
            d: input dimension
            m: hidden layer dimension 
